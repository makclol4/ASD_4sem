"""
Задача о раскладке по ящикам заключается в том, чтобы разложить определенное 
количество предметов в ящики, при этом учитывая ограничения на количество 
предметов в каждом ящике.

Метод перебора подходит для решения данной задачи, так как мы можем перебрать
все возможные варианты раскладки предметов по ящикам и выбрать тот, который 
удовлетворяет ограничениям.
"""

"""
Функция `distribute_items` принимает три параметра: количество предметов,
 количество ящиков и максимальное количество предметов в одном ящике. 
 Она вызывает вспомогательную функцию `distribute_recursive` для рекурсивного
 распределения предметов по ящикам. Если распределение невозможно, функция 
 возвращает None, иначе список, соответствующий распределению предметов по ящикам.
"""
